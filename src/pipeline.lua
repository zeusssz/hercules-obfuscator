...
function Pipeline.process(code)
    ...
    if config.get("settings.dynamic_code.enabled") then -- todo
    ...
    if config.get("settings.function_inlining.enabled") then -- todo
    ...
    if config.get("settings.string_encoding.enabled") then -- yes after the vm, its much better like that and check the string encoder file to see why (the top comment I left)
    ...
    if config.get("settings.variable_renaming.enabled") then
        ...
        local max_length = config.get("settings.variable_renaming.max_length")
